...
2. 针对错题提供详细的解析和学习建议  
3. 给出下一步的学习指导和练习建议
4. 语言要亲切、鼓励，避免过于严厉的批评
5. 如果用户表现优秀，也要给予肯定和进阶建议

请以专业但友好的语气回答，帮助用户更好地理解和掌握相关知识点。
`;
...
export function buildAnalysisPrompt(context: {
  ...
  if (quizData && quizData.length > 0) {
    quizContent = quizData.map((q: any) => {
  ...
  if (userRecord && userRecord.length > 0) {
    performanceContent = userRecord.map((record: any) => {
      const quiz = quizData.find((q: any) => q.id === record.exerciseId);
      const quizTitle = quiz ? quiz.exerciseTitle : `习题ID: ${record.exerciseId}`;
      const userAnswer = record.selectedOptionId ? 
        `选项ID: ${record.selectedOptionId}` : 
        (record.blankAnswer ? `填空答案: "${record.blankAnswer}"` : '未作答');
